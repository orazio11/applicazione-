...
<body>
    ...
    <script type="text/babel">
        ...
        const INITIAL_FORM_STATE = { name: '', category: 'Nessuna Categoria', sets: '3', reps: '10', rest: '60"', notes: '', imageUrl: '', videoUrl: '' };
            ...
            return (
                <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700/50 shadow-lg">
                    ...
                    <div className="p-4 flex flex-col sm:flex-row gap-4">
                        {exercise.imageUrl && (
                            <img src={exercise.imageUrl} className="w-full sm:w-24 h-48 sm:h-24 object-cover rounded flex-shrink-0" crossOrigin="anonymous" onError={(e) => { e.target.style.display = 'none'; }} />
                        )}
                        <div className="flex-1">
                            <div className="flex items-center justify-between">
                                <h3 className="font-teko text-2xl font-semibold text-white uppercase tracking-wide">{exercise.name}</h3>
                                {exercise.videoUrl && <a href={exercise.videoUrl} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-blue-600/50 hover:bg-blue-500/50 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white" viewBox="0 0 16 16"><path d="m11.596 8.697-6.363 3.692c-.54.313-1.233-.066-1.233-.697V4.308c0-.63.692-1.01 1.233-.696l6.363 3.692a.802.802 0 0 1 0 1.393z"/></svg></a>}
                            </div>
                            ...
                            <div className="flex items-center gap-6 mt-3 text-white text-center">
                                ...
                                <div><div className="text-xs text-gray-400">REPS</div><div className="font-bold text-lg">{exercise.reps}</div></div>
                        ...
                        <table className="w-full text-center text-xs">
                             ...
                             <tbody>
                                 {(exercise.tracking || []).map((weekData, weekIndex) => (
                                     <tr key={weekIndex}>
                                         <td className="p-1 h-12 align-top"><input type="text" className="tracking-input h-full" value={weekData.load || ''} onChange={(e) => handleInputChange(weekIndex, 'load', e.target.value)} /><div className="text-gray-500 text-[10px] leading-tight mt-0.5">{weekIndex > 0 && exercise.tracking[weekIndex-1].load ? `Prec: ${exercise.tracking[weekIndex-1].load}`: '\u00A0'}</div></td>
                                         <td className="p-1 h-12 align-top"><input type="text" className="tracking-input h-full" value={weekData.reps || ''} onChange={(e) => handleInputChange(weekIndex, 'reps', e.target.value)} /><div className="text-gray-500 text-[10px] leading-tight mt-0.5">{weekIndex > 0 && exercise.tracking[weekIndex-1].reps ? `Prec: ${exercise.tracking[weekIndex-1].reps}`: '\u00A0'}</div></td>
                                         <td className="p-1 h-12 align-top"><input type="text" className="tracking-input h-full" value={weekData.notes || ''} onChange={(e) => handleInputChange(weekIndex, 'notes', e.target.value)} /></td>
                                     </tr>
                                 ))}
                             </tbody>
                        </table>
        ...
        const WorkoutSheet = ({ clientName, sheetData, logoUrl, sheetId, onTrackingChange }) => {
            return (
                <div id={`printable-area-${sheetId}`} className="printable-area bg-gray-900 text-white shadow-2xl rounded-lg overflow-hidden font-sans mb-8">
                    ...
                    <main className="p-2 sm:p-4 space-y-4">{sheetData.exercises.map((exercise, index) => (<SingleExerciseBlock key={exercise.id} exercise={exercise} onTrackingChange={(exId, weekIndex, field, value) => onTrackingChange(sheetId, exId, weekIndex, field, value)} />))}</main>
                </div>
            );
        };
        ...
        const ExerciseForm = ({ onAddOrUpdate, currentExercise, setCurrentExercise, imageLibrary, updateImageLibrary }) => {
            ...
            return (<form onSubmit={handleSubmit} className="space-y-4 p-6 bg-gray-800 rounded-lg"><h3 className="text-2xl font-bold text-white font-teko tracking-wide">{currentExercise ? 'Modifica Esercizio' : 'Aggiungi Esercizio'}</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="md:col-span-2"><label className="block text-sm font-medium text-gray-300">Nome Esercizio</label><input type="text" name="name" value={formState.name} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500" required /></div><div><label className="block text-sm font-medium text-gray-300">Categoria</label><select name="category" value={formState.category} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500">{EXERCISE_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div><div><label className="block text-sm font-medium text-gray-300">URL Immagine</label><input type="text" name="imageUrl" value={formState.imageUrl} onChange={handleChange} onPaste={handlePaste} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500" placeholder="Incolla URL o immagine"/></div><div className="md:col-span-2"><label className="block text-sm font-medium text-gray-300">URL Video (Opzionale)</label><input type="text" name="videoUrl" value={formState.videoUrl || ''} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500" placeholder="es. https://youtube.com/..."/></div><div className="grid grid-cols-3 gap-2"><div><label className="block text-sm font-medium text-gray-300">Serie</label><input type="text" name="sets" value={formState.sets} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500" /></div><div><label className="block text-sm font-medium text-gray-300">Reps</label><input type="text" name="reps" value={formState.reps} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500" /></div><div><label className="block text-sm font-medium text-gray-300">Recupero</label><input type="text" name="rest" value={formState.rest} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500" /></div></div><div className="md:col-span-2"><label className="block text-sm font-medium text-gray-300">Note Generali</label><textarea name="notes" rows="2" value={formState.notes} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-blue-500 focus:border-blue-500"></textarea></div></div><div className="flex justify-end space-x-3">{currentExercise && (<button type="button" onClick={() => setCurrentExercise(null)} className="py-2 px-4 bg-gray-600 hover:bg-gray-500 text-white font-semibold rounded-lg shadow-md">Annulla</button>)}<button type="submit" className="py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg shadow-md">{currentExercise ? 'Aggiorna' : 'Aggiungi'}</button></div></form>);
        };
        ...
        const WorkoutTrackerApp = ({ auth, userId, db }) => {
            ...
            // Handlers e funzioni di utilità
            ...
            const handleWeeksChange = (e) => { const numWeeks = Math.max(1, Math.min(8, parseInt(e.target.value, 10) || 4)); setAppDataState(prev => { const updatedSheets = {...prev.sheets}; updatedSheets[activeSheetId].weeks = numWeeks; updatedSheets[activeSheetId].exercises.forEach(ex => { const newTracking = Array.from({length: numWeeks}, (_, i) => ex.tracking?.[i] || {load: '', reps: '', notes: ''}); ex.tracking = newTracking; }); return {...prev, sheets: updatedSheets}; }); };
            const updateImageLibrary = (name, url) => {setAppDataState(prev => ({ ...prev, imageLibrary: { ...prev.imageLibrary, [name]: url } }));};
            const handleAddOrUpdateExercise = useCallback((exerciseData) => { const numWeeks = sheets[activeSheetId].weeks || 4; setAppDataState(prev => { const currentExercises = prev.sheets[activeSheetId].exercises; const newExercises = exerciseData.id ? currentExercises.map(ex => { if (ex.id === exerciseData.id) { const updatedEx = {...exerciseData}; const newTracking = Array.from({length: numWeeks}, (_, i) => ex.tracking?.[i] || {load:'',reps:'',notes:''}); updatedEx.tracking = newTracking; return updatedEx; } return ex; }) : [...currentExercises, { ...exerciseData, id: Date.now(), tracking: Array.from({ length: numWeeks }, () => ({ load: '', reps: '', notes: '' })) }]; const newSheets = { ...prev.sheets, [activeSheetId]: { ...prev.sheets[activeSheetId], exercises: newExercises }}; return { ...prev, sheets: newSheets }; }); setCurrentExercise(null);}, [activeSheetId, sheets]);
            ...
            return (
                <>
                    <ConfirmationModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={modalAction.action} title={modalAction.title}>{modalAction.message}</ConfirmationModal>
                                ...
                                <div className="lg:sticky top-8">
                                    ...
                                    <div className="bg-gray-800 rounded-lg mb-6"><div className="flex border-b border-gray-700">{Object.keys(sheets).map(id => (<button key={id} onClick={() => setActiveSheetId(id)} className={`flex-1 p-3 font-teko text-xl sm:text-2xl tracking-wider ${activeSheetId === id ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{id}</button>))}</div>
                                        <div className="p-4 space-y-4">
                                            <div><label className="block text-sm font-medium text-gray-300">Nome Piano Specifico</label><input type="text" value={sheets[activeSheetId].name || ''} onChange={handleSheetNameChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white" /></div>
                                            <div><label className="block text-sm font-medium text-gray-300">Numero Settimane (1-8)</label><input type="number" min="1" max="8" value={sheets[activeSheetId].weeks || 4} onChange={handleWeeksChange} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white" /></div>
                                        </div>
                                    </div>
